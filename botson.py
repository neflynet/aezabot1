...
3️⃣ Выбери нужное количество и оплати своей обычной банковской картой
...
async def cmd_add_promo(message: types.Message):
    ...
    try:
        ...
        await message.answer("❌ Скидка должна быть числом (0.5 для 50%), а лимит - целым числом.")
        return
...
async def ask_promo(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("🎟️ Введи свой промокод ниже (одним сообщением):")
...
async def process_promo(message: types.Message, state: FSMContext):
    ...
    if not promo:
        await message.answer("❌ Этот промокод недействителен, истёк или достиг лимита активаций. Попробуй другой или напиши админу 💌")
        return
    
    # Применяем скидку пользователю, НО не списываем лимит, пока он не оплатит!
    await apply_promo_to_user(message.from_user.id, code, promo["discount"])
    
    await message.answer(f"🎉 Отлично! Промокод `{code}` активирован.\nТвоя скидка {int(promo['discount']*100)}% применена!\n\nСейчас перенаправлю тебя к оплате 👇")
    
    # Имитируем нажатие кнопки покупки, чтобы показать новую цену
...
async def buy_private(callback: types.CallbackQuery):
    ...
    if user["active_promo"]:
        promo_text = f"🔥 АКТИВИРОВАН ПРОМОКОД: {user['active_promo']}\nСкидка {int(discount*100)}% уже применена!"
    elif discount > 0:
        discount_text = f"🎁 Твоя реферальная скидка: {int(discount*100)}%"
    ...
    # Если промокод еще не применен, показываем кнопку
...
async def check_crypto_payment(callback: types.CallbackQuery):
    ...
    try:
        ...
        if invoice.status == "paid":
            ...
            user = await get_user(user_id) # Получаем данные для проверки промокода
            
            await activate_subscription(user_id, days=30)
            
            # 🔥 ВАЖНО: Списываем использование промокода ТОЛЬКО после успешной оплаты
